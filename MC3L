...
local M = LPC:GetMouse()
local RS = game:GetService("RunService")
local TXS = game:GetService("TextService")
local HS = game:GetService("HttpService")
local CTP = game:GetService("ContentProvider")
local ThS = loadstring(game:HttpGet("https://raw.githubusercontent.com/profileaccount/scripts/refs/heads/main/ThS", true))()

task.spawn(function()
    for _,v in pairs(ThS) do
        local LPP = Instance.new("Decal")
        LPP.Texture = v.Logo
        local BP = Instance.new("Decal")
        BP.Texture = v.Background
        CTP:PreloadAsync({LPP, BP})
    end
end)

local RbS = 7
local Th = ThS[1]
local TK = "LeftControl"
local IF = false
local IL = {}
...
function R(Button)
    ...
    task.spawn(function()
        ...
        NewCircle.Position = UDim2.new(0, M.X - NewCircle.AbsolutePosition.X, 0, M.Y - NewCircle.AbsolutePosition.Y)
...
function Library:Main(GName)
    ...
    UIRevamp.Name = rS(math.random(15, 25))
    ...
    MainBackground.Image = "rbxassetid://110244276576837"
    D(MainBackground)

    UIS.InputBegan:connect(function(v)
        if (v.KeyCode.Name == "RightControl" or v.KeyCode.Name == TK) and not IF then 
    ...
    Logo.Image = "rbxassetid://86332603662671"
    ...
    function TabLibrary:Notify(NText, Time)
        ...
        NotificationName.Text = "mike's cheat v3"
        ...
        NotificationLogo.Image = "rbxassetid://86332603662671"
        ...
        local function organise()
            ...
            for i = #notifies, 1, -1 do
                local tween = TS:Create(notifies[i], TweenInfo.new(0.20, Enum.EasingStyle.Sine), {
        ...
        NotificationClose.InputBegan:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 then
                local tween = TS:Create(Notification,
                                                    TweenInfo.new(0.25,
                                                                  Enum.EasingStyle
                                                                      .Sine), {
        ...
        local tween1 = TS:Create(NotificationBar, TweenInfo.new(
                                                 Time, Enum.EasingStyle.Linear),
                                             {Size = UDim2.new(0, 2, 0, 6)})
        tween1:Play()
        tween1.Completed:Connect(function()
            local tween2 = TS:Create(Notification, TweenInfo.new(
    ...
    function TabLibrary:NewTab(TName)
        ...
        TabToggle.BackgroundColor3 = Th.Color1
        ...
        if FirstTab then
            TabToggle.BackgroundColor3 = Th.Color2
            ...
            if input.UserInputType == Enum.UserInputType.MouseButton1 then
                for i, v in pairs(TabContainer:GetChildren()) do
                    if v ~= TabUIListLayout then
                        v.TabToggle.BackgroundColor3 = Th.Color1
                    end
                end
        ...
        function ItemLibrary:NewButton(BName, callback)
            ...
            ButtonButton.BackgroundColor3 = Th.Color1
        ...
        function ItemLibrary:NewToggle(TGName, callback, State)
            ...
            if not CurrentState then
                ToggleToggle.BackgroundColor3 = Th.Color1
            else
                ToggleToggle.BackgroundColor3 = Th.Color2
            end
        ...
        function ItemLibrary:NewSlider(SName, MinVal, MaxVal, Inc, callback, StartValue)
            ...
            UIS.InputChanged:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseMovement and SDragging == true then
                    set(MinVal + ((MaxVal - MinVal) * math.clamp((M.X - SliderBackground.AbsolutePosition.X) / SliderBackground.AbsoluteSize.X, 0, 1)))
                end
            end)
            local SliderLibrary = {}
        ...
        function ItemLibrary:NewKeybind(KName, Key, callback)
            ...
            Keybind.BackgroundColor3 = Th.Color1
        ...
        function ItemLibrary:NewDropdown(DName, Items, callback, ShowItem, UseItem)
            ...
            DropdownToggle.ImageColor3 = Th.Color1
            DropdownToggle.ScaleType = Enum.ScaleType.Crop
            table.insert(IL, DropdownToggle)
            ...
            local function closeOpen()
                if not Open and (#Items - 1) > 0 then
                    ...
                    local tween = TS:Create(DropdownContainerFrame, TweenInfo.new(0.3, Enum.EasingStyle.Sine), {
                    ...
                    local tween1 = TS:Create(DropdownToggle, TweenInfo.new(0.3, Enum.EasingStyle.Linear,Enum.EasingDirection.In, 0), {Rotation = 90})
                    tween1:Play()
                else
                    Open = false
                    for _, v in pairs(DropdownContainerFrame:GetChildren()) do
        ...
        function ItemLibrary:NewColorpicker(CName, callback, Color)
            ...
            RainbowToggle.BackgroundColor3 = Th.Color1
            ...
            ColorpickerToggle.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 then
                    closeOpen()
                    R(ColorpickerToggleFrame)
                end
            end)

            local function set(h, s, v)
                local tween = TS:Create(HueButton, TweenInfo.new(0.05,Enum.EasingStyle.Sine),{Position = UDim2.new(0.5, 0, h, 0)})
                tween:Play()
                local tween1 = TS:Create(SatButton, TweenInfo.new(0.05,Enum.EasingStyle.Sine),{Position = UDim2.new(s, 0, 1 - v, 0)})
                tween1:Play()
            ...
            HueBase.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 and
                    ...
                    MC = M.Move:Connect(function()
                        set(math.clamp(
                                (M.Y - HueBase.AbsolutePosition.Y) /
            ...
            SatBase.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 and
                    ...
                    MC = M.Move:Connect(function()
                        set(Ch,
                            math.clamp(
                                (M.X - SatBase.AbsolutePosition.X) /
                                    SatBase.AbsoluteSize.X, 0, 1), 1 -
                                math.clamp(
                                    (M.Y - SatBase.AbsolutePosition.Y) /
            ...
            RainbowToggle.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 then
                    ...
                    if not IsRainbow then
                        ...
                        RainbowToggle.BackgroundColor3 = Th.Color2
                        task.spawn(function()
                            while IsRainbow do
                                set(tick() % RbS / RbS, Cs, Cv)
                                wait()
    ...
    function TabLibrary:NewConfigTab(CustomCredits)
        local Path = "MC3/" .. game.GameId
        if not isfolder("MC3") then makefolder("MC3") end
        if not isfolder(Path) then
            makefolder(Path)
        end
        local DefaultConfig = Config
        if DefaultConfig then
            writefile(Path.."/default.json", HS:JSONEncode(DefaultConfig))
        end
        ...
        ConfigTab:NewKeybind("Toggle UI", Enum.KeyCode[TK], function(v)
            TK = v.Name
        end)
        ...
        ConfigTab:NewButton("Load", function()
            local c = HS:JSONDecode(readfile(Path.."/"..string.lower(SelectedConfig)))
        ...
        ConfigTab:NewButton("Overwrite", function()
            if string.lower(SelectedConfig) == "default.json" then
                ...
                writefile(Path.."/"..string.lower(SelectedConfig),HS:JSONEncode(Config))
            end
        end)
        ...
        ConfigTab:NewButton("Create", function()
            if isfile(Path.."/"..string.lower(NM)..".json") or string.lower(NM) == "default" then
                ...
                writefile(Path.."/"..string.lower(NM)..".json",HS:JSONEncode(Config))
                configs = {}
                ConfigDropdown:Clear()
                wait(0.4)
                for _, v in pairs(listfiles("MC3/"..game.GameId)) do
                    table.insert(configs, string.split(v,"MC3/"..game.GameId.."\\")[2])
                    ConfigDropdown:AddItem(tostring(string.split(v,"MC3/"..game.GameId.."\\")[2]))
                end
                ConfigDropdown:Set(configs[1])
            end
        end)
        CustomCredits = CustomCredits or tostring(game:HttpGet("https://raw.githubusercontent.com/profileaccount/scripts/refs/heads/main/C"))
